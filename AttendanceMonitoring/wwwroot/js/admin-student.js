...
$(document).ready(function () {
    ...
    $(document).on('submit', '#EditStudentForm', function (e) {
        ...
        $('#EditStudentForm .form-control').removeClass('border-danger');
        $('#EditStudentForm .validation-error-message').remove();

        $.ajax({
            url: '/Admin/EditStudent/' + userID,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function (xhr, status, error) {
                ...
                loadSpinner();
                loadPageBlur();
                showDangerToast('An error occurred while updating the student.');
            }
        });
    });
